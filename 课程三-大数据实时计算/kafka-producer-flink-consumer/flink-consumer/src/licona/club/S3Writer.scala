...
class S3Writer(accessKey: String, secretKey: String, endpoint: String, bucket: String, keyPrefix: String, period: Int) extends OutputFormat[ObjectNode] {
  var timer: Timer = _
  var file: File = _
  ...
  override def writeRecord(it: ObjectNode): Unit = {
    this.synchronized {
      if (StringUtils.isNoneBlank(it.toString)) {
        if (fileWriter == null) {
